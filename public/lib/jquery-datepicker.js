...
var topY = 0, leftX = 0;
//自定义定位偏移量 2007-02-11 由 寒羽枫添加 //选择日期 →
...
Calendar.prototype.ReturnDate = function(dt) {
	...
	//找出函数的字串
	ev = ev.substring(((ev.indexOf("ValidatorOnChange();") > 0) ? ev.indexOf("ValidatorOnChange();") + 20 : ev.indexOf("{") + 1), ev.lastIndexOf("}"));
...
Calendar.prototype.bindData = function() {
	...
	for (var i = 0; i < tds.length; i++) {
		...
		if (dateArray[i] != "&nbsp;") {
			...
			// 2014-08-04 增加双击确定 @张永忠
			tds[i].ondblclick = function() {
				var today = (calendar.DateMode != pickMode["day"]) ? new Date(calendar.year, calendar.month, calendar.day, calendar.hour, calendar.minute, calendar.second) : new Date();
				calendar.ReturnDate(today.format(calendar.dateFormatStyle));
			}
			tds[i].style.cursor = "pointer";
			//2007-08-06 由寒羽枫添加，鼠标变成手指状
			tds[i].onmouseover = function() {
